...
def main():
    ...
    parser.add_argument("--finetuned_model_dir", default=None, type=str,
                        help="Bert fine-tuned language model dir using domain corpus")
    ...
    if args.finetuned_model_dir is not None:
        model = BertForSequenceClassification.from_pretrained(args.finetuned_model_dir, num_labels=num_labels)
    else:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        with open(os.path.join(args.data_dir, "val.txt"), 'r', encoding="utf-8") as f:
            lines = f.readlines()
        
        label_ids = all_label_ids.numpy()
        for i in range(len(eval_examples)):
            if preds[i] != label_ids[i]:
                print('real: ', label_ids[i], 'predict: ', preds[i], lines[i].encode('unicode_escape'))   
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):    
        ...
        print("Accuracy...")
        print(metrics.accuracy_score(all_label_ids.numpy(), preds))
        print("Precision, Recall and F1-Score...")
        print(metrics.classification_report(all_label_ids.numpy(), preds, digits=4))
